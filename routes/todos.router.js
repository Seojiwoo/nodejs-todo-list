...
router.patch('/todos/:todoId', async (req, res, next) => {
  ...
  return res.status(200).json({ todo: currentTodo });
});
...
//     // // 1. 해야할 일 목록 조회를 진행한다.
//     const todos = await Todo.find({ order: { $gt: 20 } }).sort('-order').exec();
//     // // 2. 해야할 일 목록 조회 결과를 클라이언트에게 반환한다.
//     // const todos = await Todo.findOne({ order: { $gt: 20 } }).sort('-order').exec();
//     // const todos = await Todo.findById('65d54a73bfec50c85cda77fc').exec();
//     return res.status(200).json({ todos });
// });
